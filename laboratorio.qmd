...
# 1. Cargamos los datos limpios de la Parte 2 (Data Wrangling)
url_part2 = "src/laboratorio/datasets/dataset_part_2.csv"
df = pd.read_csv(url_part2)

# 2. Cargamos los datos con codificación One-Hot para Machine Learning
url_part3 = "src/laboratorio/datasets/dataset_part_3.csv"
X = pd.read_csv(url_part3)

# 3. Preparamos la base de datos SQL
url_sql = "src/laboratorio/datasets/Spacex.csv"
df_sql = pd.read_csv(url_sql)
con = sqlite3.connect("spacex.db")
df_sql.to_sql("SPACEXTBL", con, if_exists='replace', index=False, method="multi")
# Limpiamos filas nulas para SQL
...
![Arquitectura de adquisición de datos](img\img-1.png){width="200"}
...
![Flujo de extracción mediante API](img\img-2.png){width="200"}
...
![Proceso de extracción desde Wikipedia](img\img-3.png){width="200"}
...
![Flujo del análisis exploratorio](img\img-4.png){width="600"}
...
Figura X. Masa de carga útil frente a resultado del aterrizaje.

{{< embed src/laboratorio/jupyter-labs-eda-dataviz-v2.ipynb#task2 >}}
...
Figura X. Éxito de aterrizaje por centro de lanzamiento.
...
Figura X. Tasa de éxito por tipo de órbita.

{{< embed src/laboratorio/jupyter-labs-eda-dataviz-v2.ipynb#task4 >}}
...
Figura X. Evolución anual de la tasa de éxito.

{{< embed src/laboratorio/jupyter-labs-eda-dataviz-v2.ipynb#task5 >}}
...
```{python}
q3 = pd.read_sql_query("SELECT SUM(PAYLOAD_MASS__KG_) as 'Peso Total NASA (kg)' FROM SPACEXTABLE WHERE Customer LIKE '%NASA%';", con)
...
Tabla X. Capacidad media de carga del Falcon 9 v1.1

Indicador	ValorPromedio de carga (kg)	XXX
### Interpretación
...
Tabla X. Primer aterrizaje exitoso en tierra

Evento	FechaPrimer aterrizaje terrestre	XXX
### Interpretación
...
Tabla X. Recuperaciones exitosas en droneship

Booster	PayloadXXX	XXX
Interpretación
...
Tabla X. Distribución histórica de resultados

Resultado	TotalSuccess	XXX
Failure	XXX
...	...
[ZONA PARA GRÁFICO]

Figura X. Distribución de aterrizajes Falcon 9

## GRAFICO DE BARRAS RESULTADOS SQL
...
Tabla X. Misiones con carga máxima

Booster	Carga MáximaXXX	XXX
Interpretación
...
Tabla X. Fallos en droneship durante 2015

Mes	Booster	SitioXXX	XXX	XXX
### Interpretación
...
Figura X. Ranking histórico de resultados de aterrizaje

INSERTAR GRAFICO SQL RESULTADOS HISTORICOS
...
Figura 14. Flujo del análisis geoespacial

flowchart TD

A[Coordenadas Bases Lanzamiento]
B[Folium]
C[Mapa Interactivo]
D[Puntos de Interés]
E[Cálculo Distancias]
F[Conclusiones]
...
5.2 Identificación de las bases de lanzamiento
...
Instalación	UbicaciónCCAFS SLC-40	Cabo Cañaveral, Florida
...
Tabla 15. Coordenadas de las bases de lanzamiento

Base	Latitud	LongitudKSC LC-39A	XXX	XXX
CCAFS SLC-40	XXX	XXX
VAFB SLC-4E	XXX	XXX
[ZONA PARA CÓDIGO]

Código 15. Creación del mapa base

site_map = folium.Map(
    location=[29.559684888503615,
              -95.0830971930759],
    zoom_start=5
)
...
Figura 15. Localización de las bases SpaceX en Estados Unidos

INSERTAR CAPTURA DEL MAPA FOLIUM
...
5.3 Análisis de proximidad a zonas urbanas
...
Código 16. Marcadores geográficos adicionales
...
5.4 Proximidad a infraestructuras logísticas
...
Figura 17. Infraestructuras de transporte cercanas

INSERTAR CAPTURA CARRETERAS Y FERROCARRILES
...
5.5 Cálculo de distancias mediante Haversine
...
Código 17. Implementación de Haversine
...
5.6 Hallazgos estratégicos del análisis geoespacial
...
5.7 Dashboard interactivo con Plotly Dash
...
Figura 18. Arquitectura del Dashboard
...
Figura 19. Vista general del Dashboard

INSERTAR CAPTURA DEL DASHBOARD

5.8 Análisis de resultados del Dashboard
...
Class	Interpretación1	Aterrizaje exitoso
...
X representa el conjunto de variables predictoras.
Class representa el resultado esperado.
6.3 Variables predictoras utilizadas
...
Orbit_LEO	Orbit_GTO	Orbit_ISS1	0	0
0	1	0
0	0	1
[ZONA PARA FIGURA]

Figura 20. Transformación One-Hot Encoding
...
6.5 Estandarización de características

Las variables numéricas presentan escalas muy diferentes.

Por ejemplo:

FlightNumber → 1 a 100+
PayloadMass → 0 a 15000 kg
...
Código XX. Escalado de características
...
6.6 División entrenamiento-prueba
...
Tabla 12. Algoritmos evaluados

Modelo	TipoLogistic Regression	Clasificación Lineal
SVM	Clasificación por margen
Decision Tree	Clasificación basada en reglas
KNN	Clasificación basada en similitud
6.8 Optimización de hiperparámetros
...
Figura 22. Funcionamiento de Grid Search
...
Los datos se dividen en 10 subconjuntos.
Se utilizan 9 para entrenar.
Se utiliza 1 para validar.
El proceso se repite 10 veces.

Este enfoque reduce la dependencia respecto a una única partición de datos.

[ZONA PARA FIGURA]

Figura 23. Esquema de Validación Cruzada
...
[ZONA PARA TABLA REAL DE RESULTADOS]
Modelo	AccuracyLogistic Regression	0.833
SVM	0.833
Decision Tree	0.833
KNN	0.833
Interpretación
...
IBM Skills Network. Applied Data Science Capstone.
SpaceX API Documentation.
SpaceX. Falcon 9 Overview and Launch Data.
...
**Fórmula matemática para medir la distancia en el planeta Tierra (Fórmula de Haversine):** Usamos esta función en Python para medir exactamente a cuántos kilómetros de distancia estaba una ciudad del lugar de lanzamiento del cohete en nuestros mapas.
